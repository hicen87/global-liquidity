...
"""
全球流动性看板 - 数据引擎
洪灏框架: 全球流动性 = 美+欧+日(+中国) 央行资产负债表折美元加总
...
TROUGH_PROMINENCE = 4
CYCLE_MONTHS = 42
ANCHOR_DATE = '2025-12-31'   # manual模式的相位锚点(对齐洪灏: 2025-26为周期起点)
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data.json')

# 标的清单: key, yahoo代码, 中文名, 分组, 单位, SGE兜底(仅金银), 领先月数
ASSETS = [
    ('gold',   'GC=F',      '黄金',      '贵金属', 'USD/oz', 'Au99.99',  0),
    ('silver', 'SI=F',      '白银',      '贵金属', 'USD/oz', 'Ag(T+D)',  LAG_MONTHS),
...
def get_asset(yf_code, sge_sym, lag, retries=2, wait=2):
    for i in range(retries):
        try:
